...
fn variant_empty_object() {
    deserialize(r#"{"foo": null}"#, mk_foo_bar_variant_type, |de| {
        let de = de.unwrap();
        let (c, de) = de.unwrap_variant();
        assert_eq!(c, 0);
        assert!(de.is_none());
...
fn variant_unknown_object() {
    deserialize(r#"{"baz": null}"#, mk_foo_bar_variant_type, |de| {
        let err = de.unwrap_err();
        assert_eq!(
            err,
...
#[test]
fn enum_object() {
    deserialize(r#"{"bar": null}"#, mk_foo_bar_enum_type, |de| {
        let de = de.unwrap();
...
fn enum_unknown_object() {
    deserialize(r#"{"baz": null}"#, mk_foo_bar_enum_type, |de| {
        let err = de.unwrap_err();
        assert_eq!(
            err,
            r#"invalid value: string "baz", expected one of `["foo", "bar"]` at line 1 column 6"#
...
#[test]
fn option_none() {
    deserialize(r#"null"#, mk_option_u32_type, |de| {
        let de = de.unwrap();
