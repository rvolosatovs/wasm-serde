...
use bindings::exports::cosmonic::reflect::reflect::{List, ListType, Type};
...
pub fn serialize(
    ...
    let val = mk_val(&mut store, codec.cosmonic_reflect_reflect());
    codec
...
macro_rules! mk_serialize {
    ($codec:ident) => {
        #[track_caller]
        fn serialize(
            mk_val: impl core::ops::FnOnce(
                                &mut wasmtime::Store<()>,
                                &$crate::bindings::exports::cosmonic::reflect::reflect::Guest,
                            )
                                -> crate::bindings::exports::cosmonic::reflect::reflect::Value,
        ) -> std::vec::Vec<u8> {
...
pub fn deserialize<T>(
    ...
    let codec = codec
        .instantiate(&mut store)
        .expect("failed to instantiate codec");
    let ty = mk_ty(&mut store, codec.cosmonic_reflect_reflect());
    match codec
        ...
        .unwrap()
    {
        Ok(de) => f(Ok(Value {
            store: &mut store,
            reflect: codec.cosmonic_reflect_reflect(),
            value: de,
        })),
        Err(err) => {
            let err = codec
    ...
    ($codec:ident) => {
        ...
        fn deserialize<T>(
            ...
            mk_ty: impl core::ops::FnOnce(
                &mut wasmtime::Store<()>,
                &$crate::bindings::exports::cosmonic::reflect::reflect::Guest,
            ) -> crate::bindings::exports::cosmonic::reflect::reflect::Type,
